...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    var ans = points groupBy (findClosest(_, means))
    for(mean <- means){
      if(ans.get(mean) == None) ans = ans.updated(mean, List())
    }
    ans
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    var ans: GenSeq[Point] = GenSeq()
    for (mean <- oldMeans){
      ans = ans ++ GenSeq(findAverage(mean, classified(mean)))
    }
    ans
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    var flag = 0
    for (pp <- oldMeans zip newMeans){
      if(pp._1.squareDistance(pp._2) > eta) flag = 1
    }
    flag == 0
  }
