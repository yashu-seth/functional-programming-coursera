...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    var i: Int = 1
    var max: Float = -100000000
    while (i < input.length){
      var k: Float = input(i)/i
      if(k>max) max = k
      output(i) = max
      i+=1
    }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    var i: Int = from
    var maxAngle: Float = -123456789
    while(i < until){
      var k: Float = input(i)/i
      if(k>maxAngle) maxAngle = k
      output(i) = maxAngle
      i+=1
    }
    maxAngle
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if(end-from <= threshold) Leaf(from, end, upsweepSequential(input, from, end))
      ...
      val (r1, r2): (Tree, Tree) = parallel(upsweep(input, from, mid, threshold),
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    tree: Tree): Unit = tree match{
      case Leaf(i, j, v) => {
        downsweepSequential(input, output, startingAngle, i, j)
      }
  ...
  def parLineOfSight(input: Array[Float], output: Array[Float],
    threshold: Int): Unit = {
    val t: Tree = upsweep(input, 0, input.length, threshold)
